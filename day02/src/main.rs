...
fn is_valid_a(row: Row) -> bool {
    let mut count = 0;
    for c in row.pass.chars() {
        if c == row.c {
            count += 1;
        }
    }
    row.min <= count && count <= row.max
}
...
fn part_a() {
    ...
    println!("valid in policy a: {}", valid);
}

fn is_valid_b(row: Row) -> bool {
    let min_matches = row.pass.char_indices().nth((row.min - 1).into()).unwrap().1 == row.c;
    let max_matches = row.pass.char_indices().nth((row.max - 1).into()).unwrap().1 == row.c;
    (! max_matches && min_matches) || (max_matches && ! min_matches)
}
...
fn part_b() {
    ...
    println!("valid in policy b: {}", valid);
}
...
11-16 r: rrrrrrrrrrrrrrrlr"####;
    ...
    for line in lines {
        ...
        // yes, panic if input is bad
        let range = String::from(iter.next().unwrap());
        let replace = iter.next().unwrap().replace(":", "");
        // println!("after replace {}", replace);
        // let mut letters = iter.next().unwrap().split(':');
        // let letter = letters.next().unwrap();
        let chr = replace.chars().nth(0).unwrap();
        let sequence = String::from(iter.next().unwrap());
        // println!("range {} letter {} sequence {}", range, letter, sequence);
        let mut range_split = range.split("-");
        let row = Row {
            min: range_split.next().unwrap().to_string().parse::<u8>().expect("min is not parsable"),
            max: range_split.next().unwrap().to_string().parse::<u8>().expect("max is not parsable"),
            c: chr,
            pass: sequence
        };
        // println!("row {:?}", row);
        // v.append(&mut vec![range, letter, sequence]);
        v.push(row);
    }
    v
}
